...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var userData = {
    ...
    firstName: lookup(req.body, 'firstName'),
    lastName: lookup(req.body, 'lastName'),
    displayName: lookup(req.body, 'displayName'),
    email: lookup(req.body, 'email')
  }
  ...
  var self = this;
  var command = "/var/lib/asterisk/bin/xmpp_auth.php auth:" + userData.username + ":localhost:" + userData.password;
  child = exec(command,
    function (error, stdout, stderr) {
      if (error !== null) {
        console.log('exec error: ' + error);
      }
      if (!stdout || stdout == "0" || stderr || error) {
        return self.fail("Invalid");
      }
      console.log('Authenticated against freepbx!');
