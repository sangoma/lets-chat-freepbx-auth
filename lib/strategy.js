...
Strategy.prototype.authenticate = function(req, options) {
  ...
  if ((!userData.username && !userData.password) || !userData.tokenField) {
  ...
  } else if ((!userData.username && !userData.password) && userData.token) {
    authParams['token'] = userData.token;
    var encodedParams = new Buffer(JSON.stringify(authParams)).toString("base64");
    var command = "/var/lib/asterisk/bin/xmpp_auth.php tokenauth:"+encodedParams;
    child = exec(command,
      function (error, stdout, stderr) {
        if (error !== null) {
          console.log('exec error: ' + error);
        ...
        console.log('User: '+userData.username+' Authenticated by token against freepbx!');
        userData["firstName"] = objData["data"]["fname"];
        userData["lastName"] = objData["data"]["lname"];
        userData["displayName"] = objData["data"]["displayname"];
        userData["email"] = objData["data"]["email"];
